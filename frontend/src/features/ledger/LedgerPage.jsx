...
export default function LedgerPage() {
  ...
  const onDeleteContact = async () => {
    ...
    try {
      await deleteLedgerContact(contactDeleteTarget.id);
  ...
  return (
    ...
    >
          ...
          {loading ? Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-20 animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800" />) : contacts.length ? contacts.map((contact) => (
            <article key={contact.id} className="rounded-2xl border border-slate-200 p-3.5 shadow-sm dark:border-slate-700">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button type="button" className="rounded-lg bg-red-100 px-3 py-2 text-xs font-semibold text-danger dark:bg-red-900/30" onClick={() => setContactDeleteTarget(contact)}>Delete Contact</button>
              </div>
            </article>
      ...
      <BottomSheet open={Boolean(contactDeleteTarget)} onClose={() => setContactDeleteTarget(null)} title="Delete Contact">
        <div className="space-y-3">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            Delete <strong>{contactDeleteTarget?.name}</strong> from Ledger? Open ledger items for this contact will be closed and hidden from Ledger,
            while converted income and expense transactions will stay safe.
          </p>
          <div className="grid grid-cols-2 gap-2">
            <button type="button" className="rounded-xl bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200" onClick={() => setContactDeleteTarget(null)}>Cancel</button>
